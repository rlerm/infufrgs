% Para mudar a lingua principal do documento sem gerar warnings, use a linha abaixo.
% \PassOptionsToPackage{main=english}{babel}

\documentclass[
    % Use twoside para impressão em verso e anverso,
    % oneside para imporessão apenas no anverso.
	% twoside,
	oneside,
    % Vários idiomas podem ser informados para hifenização.
    % O último idioma é o principal do documento.
	english,brazil,
    % Opções específicas para a classe do instituto de informática, informam
    % o tipo de documento e o curso. Esta informação é usada para escolhar
    % várias strings importantes, usadas principalmente na folha de rosto e
    % ficha catalográfica.
...
% Mapear caracteres especiais no PDF (senão um CTRL+C pode não pegar todos
% os caracteres).
\usepackage{cmap}
% Selecao de codificação de fonte.
...
% Inicializações do Hyperref.
% Use hyperref, não estamos mais na idade das trevas onde textos grandes não
% tinham links. Os leitores agradecem.
%
% As opções adicionais servem para preencher os metadados do arquivo final.
\makeatletter
\hypersetup{%
    hidelinks,
    pdftitle={\@title},
...
% O pacote glossaries é usado aqui para gerar a lista de siglas
% (com uso do hyperref!).
\usepackage[acronym,shortcuts,nonumberlist,nomain,noredefwarn]{glossaries}
\makeglossaries

% Referências bibliograficas são feitas com o pacote abntex2cite.
% O pacote para citações não é incluído na classe, eliminando uma dependência.
\usepackage[
    alf,
...
\usepackage{framed}
% Não é indispensável, mas é muito útil.
\usepackage[obeyFinal]{todonotes}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Fim das configurações, início das informações do documento.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\titulo{Normas para apresentação de dissertações do Instituto de Informática e do PPGC}

% O comando \autor é diferente do original do AbnTeX2, recebendo separadamente
% o nome e o sobrenome.
% Isto foi necessário para a escrita da ficha catalográfica.
\autor{João}{da Silva}
% Da mesma forma, os comandos de orientador e coorientador foram modificados,
% e aceitam três partes: título, nome e sobrenome.
\orientador{Prof. Dr.}{Orientador}{da Silva}
\coorientador{Prof. Dr.}{Co-orientador}{da Silva}

\data{\number\year}
% Local e data podem ser mudados se necessário.
% \data{2014}
...
% Definição de siglas --- é o tipo de coisa que é bom deixar
% em um arquivo separado.
\newacronym{bb}{BB}{Banco do Brasil}
\newacronym{cc}{CC}{Código Civil}
\newacronym{br}{BR}{Brasil}
...
% Os documentos do Instituto de Informática não têm capa apenas folha de
% rosto --- mesmo que pelas normas da ABNT deveria ter.
...
% O uso do * serve para que esta página não seja listada no sumário, como
% manda as normas ABNT.
\listoffigures*
\cleardoublepage
...
\glssetwidest{UFRGS}
\printacronyms[title={Lista de Abreviaturas e Siglas}]
\cleardoublepage

% Inserir o sumario.
% A linha abaixo não é necessária, mas cria um link na tabela de conteúdos do
% PDF para o sumário. Os leitores agradecem.
...
formatação do documento. As informações aqui apresentadas devem ser suficientes
para formatar corretamente o documento com qualquer ferramenta de edição. Este
documento foi criado utilizando estilos. Observe isso com atenção.
...
seções o alinhamento é à esquerda. Para títulos de seções que não são numerados
(resumo, abstract, sumário, referências, listas, apêndices, anexos, etc.), o
...
São permitidas subdivisões até o 5\textsuperscript{o} nível (onde o capítulo é
o 1\textsuperscript{o} nível). Os parâmetros para formatação dos títulos e
espaçamentos nos diversos níveis de subdivisões são apresentados na
\autoref{tab:espacamentos}:
...
\begin{table}[htbp]
	...
	\fonte{\citeonline[p. 49-56]{furaste}.}
\end{table}
...
\begin{figure}[htb]
	...
	\end{center}
	\fonte{\citeonline[p.~12]{meregali}}
\end{figure}
...
% Note que a tabela é um pouco diferente do indicado no original da biblioteca.
% Foram usadas as macros e a "filosofia" do pacote booktabs. Vale a pena ler
% a documentação dele antes de sair fazendo tabelas.
\begin{table}[htb]
    \centering
...
A citação indireta ou livre (paráfrase) é aquela citação na qual se expressa o
pensamento de outra pessoa com nossas próprias palavras. Após fazer a citação,
deve-se indicar o nome do autor em letras minúsculas se estiver no corpo do
texto, e com letras maiúsculas se estiver dentro dos parênteses, juntamente com
o ano da publicação da obra em que se encontra a ideia referida. Não são
indicadas páginas já que a ideia pode estar resumida de uma obra inteira, de um
capítulo, de diversas partes ou de um conjunto delas. Desta forma (com o nome
no corpo do texto):

\begin{framed}
    Depois de analisar a situação, \citeonline{novoa1993} chegou a afirmar que
    o brasileiro ainda não está capacitado para escolher sua governante por
    causa de sua precária vocação política e da absoluta falta de escolaridade,
    já que o homem do povo, o zé-povinho, geralmente não sabe sequer em quem
    votou nas últimas eleições, não sabe sequer quem são seus governantes, não
    saber sequer quem determina seu próprio meio de sobreviver.
\end{framed}
...
Segundo Franco\footciteref{franco} (apud FURTADO, 1996, p. 5), texto texto
texto texto texto \todo{Não reproduzido}
